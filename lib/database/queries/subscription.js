...
export const getAllSubscriptions = async () => {
  ...
  const cursor = collection.find({}, { projection: { name: 1, email: 1, _id: 1, enabled: 1 } });
  const subscriptions = await cursor.toArray();
  return subscriptions
}

export const getAllEnabledSubscriptions = async () => {
  const db = await connection();
  const collection = db.collection('subscriptions');
  const cursor = collection.find({ enabled: true }, { projection: { name: 1, email: 1, _id: 1 } });
  const subscriptions = await cursor.toArray();
  return subscriptions
}

export const changeSubscriptionStatus = async (email) => {
  const db = await connection();
  const collection = db.collection('subscriptions');
  const subscription = await collection.findOne({ email });
  console.log(subscription.enabled === undefined)
  // Set the enabled field to false
  return collection.updateOne({ email }, { $set: { enabled: subscription.enabled === undefined ? false : subscription.enabled ? false : true } });
}
